  ...
  SqfEntityProvider(SqfEntityModelProvider dbModel, {String? tableName, List<String>? primaryKeyList, String? whereStr, SqfEntityConnection? connection}) {
    ...
    _connection = connection ??
        SqfEntityConnection(_dbModel!.databaseName!,
            bundledDatabasePath: _dbModel!.bundledDatabasePath, dbVersion: _dbModel!.dbVersion ?? 1, password: _dbModel!.password, databasePath: _dbModel!.databasePath);
...
List<String> checkTableIndexes(SqfEntityTableBase table) {
  ...
  for (SqfEntityFieldType field in table.fields!) {
    ...
    if (indexName != null) {
      ...
      alterTableQuery.add('CREATE UNIQUE INDEX IF NOT EXISTS $indexName ON ${table.tableName} (${columns.join(',')})');
    }
  }
  if (alterTableQuery.isNotEmpty) {
    ...
    if (tableFields.isNotEmpty) {
      ...
      for (int i = 0; i < tableFields.length; i++) {
        if (tableFields[i]['name'].toString() != primaryKeyName) {
          existingDBfields
              .add(SqfEntityFieldBase(tableFields[i]['name'].toString(), parseDbType(tableFields[i]['type'].toString()), isPrimaryKeyField: primaryKeyNames!.contains(tableFields[i]['name'])));
        }
      }
    }
